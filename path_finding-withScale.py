...
# I will test every algorithm on a 2D grid and calculate the time it takes to find endpoint, if it is reachable
        ...
        for y in range(rows):
            for x in range(cols):
                if self.labyrinth[y, x] == 1:  # Wand
                    ...
                    # Setze das mittlere Pixel auf Blau
                    pygame.draw.rect(self.surface, BLUE, (x + 4, y + 4, 2, 2))
...
class Explorer:
        ...
        if not self.path_found and not self.open_set.empty():
            ...
            for neighbor in self.get_neighbors(current):
                ...
                if temp_g_score < self.g_cost.get(neighbor, float('inf')):
                    ...
                    self.f_cost[neighbor] = temp_g_score + self.heuristic(neighbor)
    ...
    def log_step(self):
        last_position = self.came_from.get(self.current_position, "Start")
...
blue_x, blue_y = find_blue_pixel(labyrinth_instance.surface)
...
if orange_x is not None and orange_y is not None and blue_x is not None and blue_y is not None:
    explorer = Explorer(orange_x, orange_y, blue_x, blue_y, ORANGE)
else:
...
explorer.algorithm_choice = 1  # Example: Choose A* algorithm
